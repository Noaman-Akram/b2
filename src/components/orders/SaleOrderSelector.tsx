...
const SaleOrderSelector: React.FC<SaleOrderSelectorProps> = ({ onSelect, onClear }) => {
  ...
  const fetchOrders = async () => {
    if (hasFetchedOrders) return; // Don't fetch if we already have orders
    
    setLoading(true);
    try {
      console.log('[SaleOrderSelector] Fetching orders');
      ...
      console.log('[SaleOrderSelector] Fetched orders:', orderData);
      setOrders(orderData || []);
  ...
  const handleToggle = () => {
    if (!isOpen) {
      fetchOrders(); // Fetch orders when opening the selector
    }
    setIsOpen(!isOpen);
  };
